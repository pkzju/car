...
U32 Verify_CRC16_Check_Sum(U8 *pchMessage, U32 dwLength)
{
  ...
  wExpected = Get_CRC16_Check_Sum ( (U8 *)pchMessage, dwLength - 2, CRC_INIT);

  return ((U8)(wExpected & 0x00ff) == pchMessage[dwLength - 2] && ((U8)(wExpected >> 8) & 0x00ff) == pchMessage[dwLength - 1]);
}
    ...
    case S_CRC:                          // Check Header via CRC8
      {
        state     = S_IDLE;
        buffer[index] = pData;
        if (Verify_CRC8_Check_Sum(buffer, 6)) //6
        {
          index = 0;
...
unsigned char FrameUnpack(unsigned char token, unsigned char* pBuffer)
{
  ...
  {
    // Find Header Success!
    memcpy(pBuffer, pHeader, sizeof(tHeader));
    return 1;
  }
  return 0;
}
...
unsigned char FramePack(unsigned char* pData)
{
  ...
  pData[0] = PROTOCAL_tHeader;

  //append the crc8 of header at the pDataOut[sizeof(tHeader)-1]
  Append_CRC8_Check_Sum(pData, sizeof(tHeader));
